...
const InstagramSection: React.FC = () => {
  ...
  return (
    <section ref={sectionRef} id="instagram" className="bg-primary/5 py-32 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center space-y-4 mb-16">
          <p className="text-xs font-extrabold text-primary tracking-[0.2em] uppercase">Visual Inspiration</p>
          <h2 className="font-serif text-4xl sm:text-5xl font-bold text-text-main">Follow the Bloom</h2>
          <a 
            href="https://instagram.com" 
            target="_blank" 
            rel="noopener noreferrer" 
            className="inline-flex items-center gap-2 text-primary font-bold hover:opacity-70 transition-all group"
          >
            @SugarBloomAtelier
            <span className="material-symbols-outlined text-lg transition-transform group-hover:rotate-12">open_in_new</span>
          </a>
        </div>
            ...
            >
              ...
              <div className="absolute inset-0 bg-primary/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-center justify-center pointer-events-none">
                <div className="w-14 h-14 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center text-primary shadow-2xl scale-50 group-hover:scale-100 transition-transform duration-500">
                  <span className="material-symbols-outlined fill-icon text-2xl">favorite</span>
                </div>
              </div>
            </div>
          ))}
